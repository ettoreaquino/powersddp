...
def spd_result(total_cost: float,
               future_cost: float,
               hydro_units: list,
               thermal_units: list,
               final_volume: object,
    ...
    for i, hgu in enumerate(hydro_units):
        _add_row(name=hgu["name"], title='Final Volume', value=final_volume[i].value()[0])
        _add_row(name=hgu["name"], title='Turbined Volume', value=turbined_volume[i].value()[0])
        _add_row(name=hgu["name"], title='Shedded Volume', value=shedded_volume[i].value()[0])
        _add_row(name=hgu["name"], title='Water Cost', value=constraints[i].multiplier.value[0])
...
def ulp_result(stages: int,
    ...
    for stage in range(stages):
        ...
        for i, hgu in enumerate(hydro_units):
            ...
            _add_row(name=hgu["name"], title='Shedded Volume', value=shedded_volume[i][stage].value()[0])
            _add_row(name=hgu["name"], title='Water Cost', value=constraints[i].multiplier.value[0])
